 ...
 *  writes it to a text file, and sends it to the cloud:
 *  location data
 *  time data
 *  9-axis motion data
 *  water temp
 ...
 *  pressure
 *  
 *  LED lights up after data is sent
 *
 *  desired units coming soon to a repository near you!
 *  
 */
...
MS5803 sensor(ADDRESS_HIGH);  //pressure sensor
double base_altitude = 702.0; // Altitude of SparkFun's HQ in Boulder, CO. in (m)
TSYS01 airTempSensor;         //
TSYS01 waterTempSensor;       //
File myFile;                  //
const int chipSelect = A5;    //selector for SD
byte indicatorPin = A4;       //selecter for LED
...
void processData() {
  ...
  if (req != NULL)
  {
    ...
    if (body)
    {
      ...
      JAddNumberToObject(body, "pressure", pressure_abs); 
...
void loop() {
  dataSent = false; //process function has not been called
  {
...
void output(){
  ...
  serialDebug.print("pressure (mbar): ");
  serialDebug.println(pressure_abs); //output number of satellites   
...
void setUpSD(){
  ...
  if (SD.exists("buoy.txt")) {
    ...
    serialDebug.println("Creating buoy.txt...");
...
void writeToSD(){
  myFile = SD.open("buoy.txt", FILE_WRITE);
  if (myFile) {
    serialDebug.print("Writing to buoy.txt...");
    ...
    myFile.print("(lat)");
    myFile.print(lat); //output lat
    myFile.print(", ");
    myFile.print("(lon)");
    myFile.print(lon); //output lon
    myFile.print(", ");
  
    myFile.print("# of satellites: ");
    myFile.print(sats); //output number of satellites  

    myFile.print("Air Temp (C): ");
    myFile.print(temp_air); //output air temp
    myFile.print(", ");
    myFile.print("Water Temp (C): ");
    myFile.print(temp_wat); //output water temp
    myFile.print(", ");
  
    myFile.print("pressure (mbar): ");
    myFile.print(pressure_abs); //output number of satellites  
    myFile.print(", ");
    ...
    serialDebug.println("error opening buoy.txt");
  }
}
