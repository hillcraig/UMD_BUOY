...
const int WATER_CHANNEL = 0x00;
const int AIR_CHANNEL = 1;
...
unsigned long INTERVAL = 10 * 60; // 10 minutes
...
typedef enum {
  ...
  read_temp,    // Read temperature from air and water sensors
  read_IMU,      //Read and collect data from IMU
  ...
  switch (loop_step) {
    ...
    case start_GNSS:
      ...
      if (agtVbat < VBAT_LOW) {
        ...
        if (myGNSS.begin(agtWire) == false) //Connect to the u-blox module using agtWire port
        {
          ...
          loop_step = read_temp; // Move on, skip reading the GNSS fix
        }
    ...
    case read_GNSS:
      ...
      else
      {
        ...
        loop_step = read_temp; // Move on, read the pressure and temperature
      }
    ...
    */
    // ************************************************************************************************
    // Start the LTC3225 supercapacitor charger6
        case start_LTC3225:{
    ...
    case start_9603:
      ...
      else
      {
       ...
       // char pressureStr[9]; // pressure string
        ...
        char airTempStr[10]; //air temp string
        ftoa(airTemp, airTempStr, 1, 10);
        char waterTempStr[10];
        ftoa(waterTemp, waterTempStr, 1,10);
        char waveHeightStr[10];
        ftoa(waveHeight, waveHeightStr, 2,10);
        char wavePeriodStr[9];
        ftoa(wavePeriod, wavePeriodStr, 1,9);
        char waveDirectionStr[10];
        ftoa(waveDirection, waveDirectionStr, 0,10);
       ...
       /* // Assemble the message using sprintf
        sprintf(outBuffer, "%d%s%s%s%s%s,%s,%s,%s,%s,%d,%d,%d,%s,%s,%s,%d", agtYear, gnssMonth, gnssDay, gnssHour, gnssMin, gnssSec, 
          latStr, lonStr, altStr, speedStr, agtCourse, agtPDOP, agtSatellites, pressureStr, temperatureStr, vbatStr, iterationCounter);*/
            // Current message using sprintf
        sprintf(outBuffer, "%d%s%s%s%s%s,%s,%s,%s, %d,%s,%s,%s, %s, %s, %s, %d", agtYear, gnssMonth, gnssDay, gnssHour, gnssMin, gnssSec, 
          latStr, lonStr, altStr, agtSatellites, airTempStr, waterTempStr,vbatStr, waveHeightStr, wavePeriodStr, waveDirectionStr, iterationCounter);
